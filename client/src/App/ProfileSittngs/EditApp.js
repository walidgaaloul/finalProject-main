...
import { loadUser } from '../../store/actions/authActionss'
...
/*	const [firstNameState, firstnameSet] = useState('');
 ...
 //user = useSelector((state) => state.AuthReducers.user);
	...
	return (
		<>
			...
			{/* <div className="black-logo ">
				...
				<p className="airbnbTitle">RENTALS</p>
			</div> */}
			<Route
				path={`${path}`}
				exact
				component={() => (
					<CardList
						firstNameState={user.FirstName}
						lastNameState={user.LastName}
						email={user.Email}
						isLoading={isLoading}
					/>
				)}
